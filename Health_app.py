    ...
    if st.button(" 🔍 Get Recommendations"):
       if not user_query.strip():  # Ensures it's not just spaces
            ...
            if st.session_state["logged_in"]:
                save_search(st.session_state["user_id"], user_query, response)
            else:
                st.write("")
...
with tab1:
    ...
    if st.button(" 🔍 Get Recommendations"):
       if not user_query.strip():  # Ensures it's not just spaces
            ...
            st.write(response)
            if st.session_state["logged_in"]:
                save_search(st.session_state["user_id"], "Nutrigenie", user_query, response)
            else:
    ...
    with tab3:
        ...
        if st.button("🔍 Analyze My Metabolism"):
            ...
            if last_meal_datetime > current_time:
                ...
                st.write(response)
                if st.session_state["logged_in"]:
                   save_search(st.session_state["user_id"], "MetaboTrack", prompt, response)
                else:
                    st.sidebar.warning("Please Login!")
    ...
    if st.button("🍽️ Suggest Recipes"):
        if not ingredients.strip():
            ...
            if recipe_response:
                ...
                if st.session_state["logged_in"]:
                   save_search(st.session_state["user_id"], "RecipeMaster" , recipe_prompt, response)
                else:
                    st.sidebar.warning("Please Login!")
            else:
    ...
    if st.button("🛍️ Generate Shopping List"):
        if not planned_recipes.strip():
            ...
            if shopping_list_response:
                ...
                if st.session_state["logged_in"]:
                    save_search(st.session_state["user_id"], "SmartShopper", shopping_list_prompt, shopping_list_response)
                else:
                   st.sidebar.warning("Please Login!")
            else:
